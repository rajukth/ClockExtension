...
function convertEnglishDateToNepali(yy, mm, dd) {
    ...
    dateString[1] = localizeNumber(y) + " " + getNepaliMonthInString(m) + " " + localizeNumber(total_nDays) + ", " + getNepaliDayOfWeekInString(day);
    return dateString;
...
function localizeNumber(temp) {
    ...
    for (let i = 0; i < temp.length; i++) {
        localized += englishToNepaliNumber(temp[i]);
    }
    return localized;
}
...
function getNepaliTime() {
    ...
    return localizeNumber(timeString.replace('AM', 'पूर्वाह्न').replace('PM', 'अपराह्न'));
}